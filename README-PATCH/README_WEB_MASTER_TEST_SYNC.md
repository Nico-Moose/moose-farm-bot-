...
## Изменение без блокировки импорта

В этой сборке ручной импорт из WizeBot НЕ блокируется для `WEB_MASTER_USERS`.
Это значит, что `!синкферма` можно запускать повторно даже для `nico_moose`.
Используйте это осторожно: повторный импорт может перезаписать более свежий прогресс сайта данными из WizeBot.


## Hotfix: мягкий WizeBot push для `nico_moose`
- `farm_upgrade_balance_nico_moose` временно пропускается при обратном sync сайт -> WizeBot.
- Остальные ключи продолжают синкаться даже если один из custom-data апдейтов упал.
- Результат sync теперь возвращает `keys`, `skippedKeys`, `failedKeys` и детальный `results` по каждому ключу.
- `currency` всегда отправляется в конце отдельной попыткой.

...
function currentCoins(profile) {
  return Number(profile?.twitch_balance || 0) + Number(profile?.farm_balance || 0) + Number(profile?.upgrade_balance || 0);
}

function ordinaryCoins(profile) {
  return Number(profile?.twitch_balance || 0);
}
...
function renderQuickStatus(data) {
  ...
  box.innerHTML = `
    ...
    <div class="quick-status-grid">
      <span>💰 Обычные монеты: <b>${formatNumber(twitchCoins)}</b></span>
  ...
  el.innerHTML = `
    <div class="profile-card-final">
      ...
      <div class="profile-stats-final">
        ...
        <div class="stat-tile"><span>💰 Монеты</span><b>${formatNumber(twitchCoins)}</b></div>

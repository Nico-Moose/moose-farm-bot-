function ensureMainActionButtons(data) {
  ...
  if (upgrade10Btn) {
    ...
    upgrade10Btn.innerHTML = farmActive
      ? (() => { const pack = (typeof getUpgrade10Preview === 'function') ? getUpgrade10Preview(data.profile || {}) : { count: 10, totalCost: 0, totalParts: 0 }; return `🚀 Ап +10<br><small>${pack.maxed ? 'максимум' : `до +${pack.count} ур. · ${formatNumber(pack.totalCost)}💰${pack.totalParts ? ' / ' + formatNumber(pack.totalParts) + '🔧' : ''}`}</small>`; })()

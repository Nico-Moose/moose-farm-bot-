...
function calcFarmUpgradeCostPreview(level) {
...
function getUpgrade10Preview(profile, maxCount = 10) {
  const currentLevel = Number(profile?.level || profile?.farm?.level || 0);
  ...
  for (let i = 1; i <= maxCount; i++) {
    ...
    totalCost += calcFarmUpgradeCostPreview(level);
    totalParts += Number(required[String(level)] ?? required[level] ?? 0) || 0;
    count++;
  }

  return { count, totalCost, totalParts, maxed: count === 0 };
}

window.getUpgrade10Preview = getUpgrade10Preview;
...
function render(data) {
  ...
  const wbPercent = Number(wb.nextExp || 0) > 0
    ? Math.max(0, Math.min(100, Math.floor((Number(wb.exp || 0) / Number(wb.nextExp || 1)) * 100)))
    : 0;

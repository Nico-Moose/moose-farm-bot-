...
function ensureMainActionButtons(data) {
  ...
  if (upgrade10Btn) {
    ...
    const pack = (typeof getUpgrade10Preview === 'function') ? getUpgrade10Preview(data.profile || {}) : { count: 10, totalCost: 0, totalParts: 0, maxed: isMaxFarm };
    upgrade10Btn.innerHTML = `🚀 Ап +10<br><small>${isMaxFarm || pack.maxed ? 'максимум' : `до +${pack.count} ур. · ${formatNumber(pack.totalCost)}💰${pack.totalParts ? ' / ' + formatNumber(pack.totalParts) + '🔧' : ''}`}</small>`;
  }
}

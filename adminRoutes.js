...
module.exports = function (db) {
  ...
  function getBodyLogin(req, field = 'login') {
    return String(req.body?.[field] || '').toLowerCase().replace(/^@/, '');
  }
...
function restoreFarmBackup(profile) {
  ...
  router.post('/transfer-farm', (req, res) => {
    const oldLogin = getBodyLogin(req, 'oldLogin');
    const newLogin = getBodyLogin(req, 'newLogin');

module.exports = function registerAdminToolsRoutes(router, ctx) {
  const {
    db,
    getProfileByLogin,
    upsertTwitchUser,
  ...
  router.post('/transfer-farm', (req, res) => {
    ...
    if (!newProfile) {
      upsertTwitchUser({ id: `transfer:${newLogin}`, login: newLogin, display_name: newLogin, profile_image_url: '' });
  ...
  router.post('/reset-cases', (req, res) => {
    ...
    const resetOne = (profile) => {
      ...
      farm.cases.opened = 0; farm.cases.totalSpent = 0; farm.cases.totalCoins = 0; farm.cases.totalParts = 0;
      farm.lastCaseAt = 0; farm.caseCooldownUntil = 0;
      saveFarmObject(profile.twitch_id, farm);
  ...
  router.post('/set-roulette-tickets', (req, res) => {
    ...
    if (!profile) return res.status(404).json({ ok: false, error: 'Игрок не найден' });
    const farm = profile.farm || {};
    farm.roulette = farm.roulette || {}; farm.roulette.tickets = amount;
    saveFarmObject(profile.twitch_id, farm);
  ...
  router.post('/run-1to1-check', (req, res) => {
    ...
    if (!profile) return res.status(404).json({ ok: false, error: 'Игрок не найден' });
    const farm = profile.farm || {};
    const report = { login, generatedAt: Date.now(), resources: { level: profile.level, farm_balance: profile.farm_balance, upgrade_balance: profile.upgrade_balance, parts: profile.parts, license_level: profile.license_level, raid_power: profile.raid_power, protection_level: profile.protection_level, turret_level: Number(profile.turret?.level || 0) }, checks: [ { id: 'upgrade', title: 'Стоимость апов', status: 'manual', note: 'Сравнить следующий ап с WizeBot !ап.' }, { id: 'buildings', title: 'Стоимость зданий', status: 'manual', note: 'Сравнить покупку/ап каждого здания с WizeBot.' }, { id: 'income', title: 'Доход зданий', status: 'manual', note: 'Сравнить инфо фермы/сбор/оффсбор.' }, { id: 'raids', title: 'Рейды', status: 'manual', note: 'Проверить цель, силу, защиту, турель, лог.' }, { id: 'market', title: 'Рынок', status: 'manual', note: 'Проверить покупку/продажу и склад.' }, { id: 'cases', title: 'Кейсы', status: 'manual', note: 'Проверить цену, множитель, кулдаун, историю.' }, { id: 'gamus', title: 'GAMUS', status: 'manual', note: 'Проверить диапазон награды и ресет 06:00 МСК.' }, { id: 'offcollect', title: 'Оффсбор', status: 'manual', note: 'Проверить 50% дохода и КД.' }, { id: 'admin', title: 'Админ-действия', status: 'manual', note: 'Проверить изменение ресурсов, сбросы, бэкап.' } ], buildings: farm.buildings || {}, case: { lastCaseAt: farm.lastCaseAt || 0, historyCount: Array.isArray(farm.caseHistory) ? farm.caseHistory.length : 0 }, gamus: { lastGamusAt: farm.lastGamusAt || 0 }, roulette: farm.roulette || { tickets: 0 }, backups: Array.isArray(farm.adminBackups) ? farm.adminBackups.map((b) => ({ createdAt: b.createdAt, reason: b.reason, login: b.login })).slice(0, 10) : [] };
  ...
  router.get('/events', (req, res) => {
    ...
    if (login) {
      ...
      where += ' AND e.twitch_id = ?'; params.push(profile.twitch_id);
    }
  ...
  router.get('/backups', (req, res) => {
    ...
    res.json({ ok: true, login, backups: backups.map((b, index) => ({ index, createdAt: b.createdAt || 0, reason: b.reason || 'backup', login: b.login || login, level: Number(b.level || 0), farm_balance: Number(b.farm_balance || 0), upgrade_balance: Number(b.upgrade_balance || 0), parts: Number(b.parts || 0), license_level: Number(b.license_level || 0), protection_level: Number(b.protection_level || 0), raid_power: Number(b.raid_power || 0), buildings: (b.farm && b.farm.buildings) || {}, caseHistoryCount: Array.isArray(b.farm?.caseHistory) ? b.farm.caseHistory.length : 0, raidLogsCount: Array.isArray(b.farm?.raidLogs) ? b.farm.raidLogs.length : 0 })) });
  });
  ...
  router.post('/restore-backup-index', (req, res) => {
    ...
    if (block === 'balances') {
      db.prepare(`UPDATE farm_profiles SET farm_balance=?, upgrade_balance=?, parts=?, updated_at=? WHERE twitch_id=?`).run(Number(backup.farm_balance || 0), Number(backup.upgrade_balance || 0), Number(backup.parts || 0), Date.now(), profile.twitch_id);
    } else if (block === 'progression') {
      db.prepare(`UPDATE farm_profiles SET level=?, license_level=?, protection_level=?, raid_power=?, turret_json=?, updated_at=? WHERE twitch_id=?`).run(Number(backup.level || 0), Number(backup.license_level || 0), Number(backup.protection_level || 0), Number(backup.raid_power || 0), JSON.stringify(deepCloneSafe(backup.turret || {}, {})), Date.now(), profile.twitch_id);
    } else if (block === 'farm') {
      const restoredFarm = deepCloneSafe(backup.farm || {}, {}); restoredFarm.adminBackups = backups;
      ...
      if (block === 'raids') {
        ...
        currentFarm.lastRaidAt = backupFarm.lastRaidAt || 0; currentFarm.raidCooldownUntil = backupFarm.raidCooldownUntil || 0; currentFarm.shieldUntil = backupFarm.shieldUntil || 0;
      }
      if (block === 'cases') {
        ...
        currentFarm.lastCaseAt = backupFarm.lastCaseAt || 0; currentFarm.caseCooldownUntil = backupFarm.caseCooldownUntil || 0;
      }
      ...
      const restoredFarm = deepCloneSafe(backup.farm || {}, {}); restoredFarm.adminBackups = backups;
      db.prepare(`UPDATE farm_profiles SET level=?, farm_balance=?, upgrade_balance=?, total_income=?, parts=?, last_collect_at=?, farm_json=?, configs_json=?, license_level=?, protection_level=?, raid_power=?, turret_json=?, updated_at=? WHERE twitch_id=?`).run(Number(backup.level || 0), Number(backup.farm_balance || 0), Number(backup.upgrade_balance || 0), Number(backup.total_income || 0), Number(backup.parts || 0), backup.last_collect_at || null, JSON.stringify(restoredFarm), JSON.stringify(deepCloneSafe(backup.configs || {}, {})), Number(backup.license_level || 0), Number(backup.protection_level || 0), Number(backup.raid_power || 0), JSON.stringify(deepCloneSafe(backup.turret || {}, {})), Date.now(), profile.twitch_id);
    }
